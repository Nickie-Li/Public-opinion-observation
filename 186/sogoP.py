...
# # SOGOP 論壇-壽險
# http://oursogo.com/forum.php?mod=forumdisplay&fid=199&filter=author&orderby=dateline&typeid=895
...
def pageDetail(detailurl):
    ...
    if soup.select('.t_f'):
        ...
        # 抓取摘要
        summary = soup.select('.t_f')[0].text.replace("_","").replace("-","").replace("※","").replace("*","").replace("\r"," ").replace("\n"," ").replace(" ",'').replace(u'\xa0','')
        # 抓取時間
...
# def getPageLink(s,e):
# #網址輸入
#     newsurl = 'http://oursogo.com/forum.php?mod=forumdisplay&fid=199&filter=author&orderby=dateline&typeid=895&page={}'
...
#                 else:
#                     rnumber = getTodayStr()+'4'+row_number(autorn)#流水號
#                     #抓時間
...
#                     autorn += 1
#         cur.execute("INSERT INTO TimeRecord VALUES (?, ?)", ('sogoP', tp))##欄位----table-C_
#     conn.commit()
#     conn.close()
    ...
    with lite.connect('../FBWEB.sqlite')as conn:##sql
        ...
        while wannaContinue:
            ...
            if soup.select_one('.common a'):##抓連結
                ...
                wannaContinue=False
        cur.execute("UPDATE TimeRecord set last_time = ? where table_name= ?",(tp,'sogoP'))
