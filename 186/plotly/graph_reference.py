...
def get_graph_reference():
    ...
    s = resource_string('plotly', path)
    graph_reference = util.decode_unicode(_json.loads(s))
...
def get_attributes_dicts(object_name, parent_object_names=()):
    ...
    attributes_dicts = {path: util.get_by_path(GRAPH_REFERENCE, path)
...
def _get_objects():
    ...
    * additional_attributes can be used to hard-code (patch) the plot schema
    ...
    """
    objects = {}
    for node, path in util.node_generator(GRAPH_REFERENCE):
